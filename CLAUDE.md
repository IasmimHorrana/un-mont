...
- **App icons:** rasterized from the same flower motif as `public/favicon.svg` — `public/apple-touch-icon.png` (180×180, full-bleed square since iOS applies its own corner rounding) and `public/pwa-192x192.png`/`pwa-512x512.png` for Android/Chrome, plus a `maskable-icon-512x512.png` (flower scaled down to stay inside the safe zone so adaptive-icon masks don't clip the petals) generated from `public/icon-maskable.svg`. Regenerate by editing `public/icon-square.svg`/`icon-maskable.svg` and re-rastering to those same filenames — swap in real branded artwork here whenever that's ready, no other code changes needed.
