...
The app needs `VITE_SHEETDB_API_URL` set in `.env.local` (gitignored, copy from `.env.example`) — a SheetDB endpoint over a Google Sheet with columns `data, dias_restantes, flor_id, nome_flor, significado, bilhete, foto_flor, foto_polaroid, legenda_polaroid, proxima_parada_foto, proxima_parada_local, resposta_dela` (`data` column must be formatted as plain text in Sheets, or SheetDB dates will not stay in the required `YYYY-MM-DD` form; `foto_polaroid` is an optional image URL and `legenda_polaroid` an optional caption shown under it — empty rows just skip the Polaroid card / render it without a caption). Without it, the app renders a friendly `ErrorState` rather than crashing.

`foto_flor` (optional image URL, same `toDirectImageUrl` handling as `foto_polaroid`) lets a specific day override the flower artwork instead of reusing the shared species illustration looked up by `flor_id` — added because flower *names* repeat across the 183 days (the pool is only ~20 species) and the user wanted some repeats to show a genuinely different image rather than always the same file. `FlorIllustration` (`src/components/flor/FlorIllustration.jsx`) checks `fotoFlor` first and only falls back to the `flor_id` → local asset lookup when the cell is empty, so leaving it blank keeps the old shared-pool behavior for that day.
...
`resposta_dela` is the one column the app *writes* rather than only reads — the "Digite um recadinho aqui..." card (`src/components/recadinho/RecadinhoCard.jsx`, route `/recadinho`) lets her type a note and, on "Enviar Amor", `enviarRecadinho()` (`src/services/sheetdb.js`) PATCHes it into today's row (matched by the `data` column, SheetDB's `PATCH /:id/:column/:value` convention) so it shows up next to that day's other content when the spreadsheet is opened. If today's row doesn't exist yet (flower/bilhete not filled in yet), the PATCH matches nothing and the function falls back to POSTing a new row with just `data` + `resposta_dela` — the incremental-content model already tolerates partial rows like this.
...
- **Flower rendering:** `src/components/flor/FlorIllustration.jsx` renders, in priority order: (1) the row's `foto_flor` URL if set, (2) a matching file in `src/assets/flores/` via `import.meta.glob` keyed by `flor_id`, (3) `FlorPlaceholder` (a colored SVG shape) with color/meaning sourced from the local reference catalog in `src/utils/floresPool.js`. Day-to-day content is meant to flow entirely through `foto_flor` now (paste a Drive/image link, same as `foto_polaroid`) — the local `src/assets/flores/*.png` pool exists as the *default* art for each of the ~20 recurring species and only needs touching when adding/replacing a species' base illustration, not per day.
...
- **App icons:** rasterized from `src/assets/icon-app.jpg` (the hand-drawn stacked-journals artwork) — `public/apple-touch-icon.png` (180×180, full-bleed square since iOS applies its own corner rounding) and `public/pwa-192x192.png`/`pwa-512x512.png` for Android/Chrome, plus a `maskable-icon-512x512.png` (artwork scaled to ~72% and centered on its sampled background color so adaptive-icon masks don't clip the flowers/leaves at the edges). `public/favicon.svg` (the browser-tab icon) intentionally stays a separate, simpler flower mark — the journal illustration is too detailed to read at 16–32px. Regenerate the PNGs by re-rastering `src/assets/icon-app.jpg` to those same filenames whenever the artwork changes.
- **Placeholder assets still pending:** real flower PNGs (`src/assets/flores/README.txt`) — the day-to-day illustrations, not the app icon.
