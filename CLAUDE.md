...
The app needs `VITE_SHEETDB_API_URL` set in `.env.local` (gitignored, copy from `.env.example`) — a SheetDB endpoint over a Google Sheet with columns `data, dias_restantes, flor_id, nome_flor, significado, bilhete` (`data` column must be formatted as plain text in Sheets, or SheetDB dates will not stay in the required `YYYY-MM-DD` form). Without it, the app renders a friendly `ErrorState` rather than crashing.
...
- **Routing (React Router):** `/` and `/album` are nested under `PageShell` (persistent Header + BottomNav); `/flor/:data`, `/bilhete/:data`, and `/album/:data` are standalone full-screen detail routes (own `BackButton`, no bottom nav). See `src/App.jsx`.
- **Data flow:** `src/services/sheetdb.js` fetches and normalizes rows from SheetDB into `{date, diasRestantes, florId, nomeFlor, significado, bilhete, hasContent}`. `DiarioProvider` (`src/context/DiarioContext.jsx`) fetches once, caches the result in `localStorage` for 12h (`src/services/cache.js`) to protect SheetDB's free-tier request quota, and exposes it via the `useDiario()` hook. Screens never call the service directly.
...
- **Placeholder assets still pending:** real flower PNGs (`src/assets/flores/README.txt`), self-hosted fonts (`public/fonts/README.txt`), and real app icons (currently a single placeholder SVG reused for all manifest icon sizes — fine for Android/Chrome, but Safari's `apple-touch-icon` really wants a PNG before real iOS device testing).
